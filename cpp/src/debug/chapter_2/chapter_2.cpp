...
#define offset( t, f )	( (DWORD)(DWORD_PTR)&( ( (t*)0 )->f ) )
...
void delete_bus_stop()
{
    ...
    while (bus_stop)
    {
        if (stricmp(bus_stop->name, bus_no) == 0)
        {
            printf("删除班车站点成功\n");
...
void view_one_bus()
{
    ...
    _ASSERT((bus = get_bus(bus_no)) != NULL);
...
void memory_window_test()
{
    ...
    printf("test str is %s,wtest str is %S,itest %d", test_str, wtest_str, itest);
}
void print_menu()
{
    printf("********班车管理系统                        ********\n");
...
int main_bus(int argc, char* argv[])
{
    memory_window_test();
